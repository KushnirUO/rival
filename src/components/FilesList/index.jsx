...
const FilesList = () => {
    ...
    function handleFilter() {
        ...
        if(filterItem === '"1"'){setFilters(cardsAdd.filter((item) => (
            item.format === '.jpg'
            || item.format === '.bmp'
            || item.format === '.gif'
            || item.format === '.jpeg'
            || item.format === '.png')))}
        if(filterItem === '"2"'){setFilters(cardsAdd.filter((item) =>
            item.format === '.pdf'
            || item.format === '.zip'
            || item.format === '.txt'
            || item.format === '.rar'
            || item.format === '.docx'))}
        if(filterItem === '"3"'){setFilters(cardsAdd.filter((item) =>
            item.format === '.mp3'
            || item.format === '.wav'
            || item.format === '.au'))}
        if(filterItem === '"4"'){setFilters(cardsAdd.filter((item) =>
            item.format === '.mp4'
            || item.format === '.avi'
            || item.format === '.mov'
            || item.format === '.mkv'
